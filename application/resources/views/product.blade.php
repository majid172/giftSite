                ...
                <div>
                    ...
                    <form action="{{ route('cart.store') }}" method="POST">
                        ...
                        <div class="flex gap-4">
                            ...
                            <button type="submit" name="action" value="buy_now" class="flex-1 bg-amber-500 hover:bg-amber-600 text-white font-bold py-4 rounded-full shadow-lg shadow-amber-500/20 transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2 text-lg" {{ $product->stock <= 0 ? 'disabled' : '' }}>
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                                Order Now
                            </button>

                            <!-- Add to Cart (Teal/Green) -->
                            <button type="submit" name="action" value="add_to_cart" class="flex-1 bg-teal-700 hover:bg-teal-800 text-white font-bold py-4 rounded-full shadow-lg shadow-teal-700/20 transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2 text-lg" {{ $product->stock <= 0 ? 'disabled' : '' }}>
