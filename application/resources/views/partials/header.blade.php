<header class="bg-white/90 backdrop-blur-md sticky top-0 z-40 shadow-sm border-b border-stone-200 font-sans">

    <!-- ROW 1: Main Header (Logo, Search, Icons) -->
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-3 lg:py-4 flex items-center justify-between lg:gap-8 relative lg:static">

        <!-- Mobile Menu (Left on mobile, hidden on Desktop) -->
        <div class="lg:hidden flex flex-1 justify-start">
            <button class="text-stone-600" @click="mobileMenuOpen = !mobileMenuOpen">
        ...
        <!-- Logo -->
        <a href="{{ route('home') }}" class="flex items-center gap-2 flex-shrink-0 justify-center absolute left-1/2 -translate-x-1/2 lg:static lg:transform-none">
            <div
        ...
        <!-- Right Actions -->
        <div class="flex items-center justify-end gap-6 flex-shrink-0 flex-1 lg:flex-none">

            <!-- User -->
            @auth
                <a href="{{ auth()->user()->role === 'admin' ? route('admin.dashboard') : route('orders.index') }}"
                    class="group flex flex-col items-center hidden lg:flex" title="Dashboard">
    ...
    <div class="hidden lg:block border-t border-stone-200 bg-white">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-10">
            ...
            <nav
                ...
                <a href="{{ route('products.index') }}" class="hover:text-amber-500 transition">Products </a>
